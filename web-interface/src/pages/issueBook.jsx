...
import IssueBookComponent from "../component/issueBookComponent";
...
const IssueBook = () => {
  ...
  const handleIssueBook = async (bookname, username) => {
    const getDueDate = () => {
      const dueDate = new Date();
      dueDate.setDate(dueDate.getDate() + 14); // Two weeks from the current date
    ...
    try{
      const response = await axiosInstance.post(
        "book/issue",
        {
          bookname,dueDate:getDueDate(), username
        },
        axiosConfig
      );
  ...
  return (
    <div>
      <nav className="navbar navbar-expand-lg bg-body-tertiary">
        <div className="container-fluid">
          <ul className="navbar-nav me-auto mb-2 mb-lg-0 d-flex gap-3 flex-row justify-content-between">
            ...
            <li className="nav-item">
              <a className="nav-link active" href="IssueBook">
                Issue Books
              </a>
            </li>
            <li className="nav-item">
      ...
      {data.length == 0 ? "NO BOOK AVALIABLE" : <IssueBookComponent books={data} issueBook={handleIssueBook} />}
    </div>
  );
};
